...
public:
	...
	[[nodiscard]] int GetBoneID() const { return id_; }

private:
	[[nodiscard]] uint32_t GetPositionIndex(float animationTime);
	[[nodiscard]] uint32_t GetRotationIndex(float animationTime);
	[[nodiscard]] uint32_t GetScaleIndex(float animationTime);
